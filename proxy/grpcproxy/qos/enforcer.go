...
type Enforcer interface {
	SyncRule(r *QoSRule) error

	DeleteRule(r *QoSRule) error
...
type QosEnforcer struct {
	...
	rulePriList       []map[string]*QoSRule
}
...
func NewQoSEnforcer(lg *zap.Logger) Enforcer {
	return &QosEnforcer{
		...
		rulePriList:       make([]map[string]*QoSRule, RulePriorityMax+1),
	}
}

func (qe *QosEnforcer) SyncRule(r *QoSRule) error {
	qe.mu.Lock()
	defer qe.mu.Unlock()
...
func (qe *QosEnforcer) DeleteRule(r *QoSRule) error {
	qe.mu.Lock()
	defer qe.mu.Unlock()
	if r.Priority < RulePriorityMin || r.Priority > RulePriorityMax {
		return ErrInvalidPriorityRule
...
func (qe *QosEnforcer) matchRule(r *RequestContext) (*QoSRule, error) {
	matched := false
