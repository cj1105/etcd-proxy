...
type QuotaIndex interface {
	...
	Get(key string) (quota *NamespaceQuota)
	...
	GetParentQuotas(key string) (quotas []*NamespaceQuota)
	...
	Delete(key string) (quota *NamespaceQuota)
	// CheckQuotaPath check the tree path for any key exceeding quota
	CheckQuotaPath(key string, valueDiff uint64, keyDiff uint64) (quotas *NamespaceQuota)
	// UpdateQuotaPath update the quota
	UpdateQuotaPath(key string, valueDiff int, keyDiff int)
}
...
func (q *quotaTreeIndex) Get(key string) (quota *NamespaceQuota) {
	if item, isFound := q.tree.Get([]byte(key)); isFound {
...
func (q *quotaTreeIndex) Put(quota *NamespaceQuota) (updatedQuota *NamespaceQuota, isUpdated bool) {
	tree, item, _ := q.tree.Insert([]byte(quota.Key), quota)
	q.tree = tree
	if item == nil {
...
func (q *quotaTreeIndex) Delete(key string) (quota *NamespaceQuota) {
	newTree, item, isDeleted := q.tree.Delete([]byte(key))
	if isDeleted {
		q.tree = newTree
		return item.(*NamespaceQuota)
	}
...
func (q *quotaTreeIndex) CheckQuotaPath(key string, byteDiff uint64, keyDiff uint64) (quotas *NamespaceQuota) {
	...
	q.tree.Root().WalkPath([]byte(key), f)
	// this will be nil if no quota was violated
	return currQuota
}

func (q *quotaTreeIndex) GetParentQuotas(key string) (quotas []*NamespaceQuota) {
	f := iradix.WalkFn(func(key []byte, value interface{}) bool {
		if value != nil {
			item := value.(*NamespaceQuota)
...
func (q *quotaTreeIndex) UpdateQuotaPath(key string, byteDiff int, keyDiff int) {
	f := iradix.WalkFn(func(key []byte, value interface{}) bool {
